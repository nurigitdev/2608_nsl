...
class RuntimeEngine:
    ...
    async def execute(
        ...
        try:
            ...
            return self._failed(ctx, "AUTHORIZATION_DENIED", "AUTHORIZATION", str(error))
        except PermissionError as error:
            return self._failed(ctx, "REPLAY_ACCESS_DENIED", "AUTHORIZATION", str(error))
        except ToolExecutionError as error:
            return self._failed(ctx, error.code, "TOOL", str(error), ExecutionStatus.TOOL_ERROR)
            ...
            return self._failed(ctx, "RUNTIME_ERROR", "RUNTIME", str(error))

    def _preflight(self, skill: SkillObject) -> None:
        if skill.ir_version != "1.0" or skill.language_version != "0.1":
            raise RuntimeError("unsupported IR or language version")
        if skill.semantics_profile != "NSL-0.1-STRICT":
            raise RuntimeError("unsupported semantics profile")
        ...
        if not skill.analysis.bounded:
            raise RuntimeError("unbounded skill")
        for required in skill.required_tools:
            ...
            if contract.contract_hash != required.contract_hash:
                raise RuntimeError(f"tool contract mismatch: {required.tool_id}")
            if required.capability != "READ":
                raise RuntimeError("WRITE capability is forbidden")
    ...
    def _bind_inputs_and_contexts(self, ctx: _ExecutionContext) -> None:
        for spec in ctx.skill.inputs:
            if spec.required and spec.name not in ctx.request.inputs:
                raise RuntimeError(f"missing required input: {spec.name}")
    ...
    def _validate_runtime_type(self, value: Any, type_info: TypeRef, name: str) -> None:
        ...
        if not valid:
            raise RuntimeError(f"runtime type mismatch for {name}")
        ...
        for statement in statements:
            ...
            if isinstance(statement, LetStatement):
                ...
                if predicate.type_info != BOOL:
                    raise RuntimeError("CHECK predicate is not Bool")
    ...
    async def _evaluate(
        ...
        if isinstance(expression, CallExpr):
            ...
            if expression.function != "sum":
                raise RuntimeError(f"unsupported built-in: {expression.function}")
    ...
    def _failed(
        ...
        return self._result(
            ctx, status, RuntimeErrorInfo(code, category, message)
        )
